...
const useOutgoing = () => {
  ...
  const generatePdf = (dataTable: any) => {
    ...
    const titleSave = `surat_jalan_${moment(new Date()).format(
      "YYYYMMDDhhmm"
    )}.pdf`;
  ...
  const queryPostOutoging = useMutation(
    (data: IData) =>
      outgoingService({
  ...
  useQuery(
    ...
    () =>
      barangService({
        ...
        }/api/barang/search?nomorMaterial=${filter.keyword}&namaMaterial=${
          filter.keyword
        }&type=outgoing`,
      }),
    {
